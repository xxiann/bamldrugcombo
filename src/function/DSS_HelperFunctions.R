#' modified version that return QC variable: believe_DSS, cond
#' codes taken from yingjchen/DSS-v2.0
#' original codes are commented and not removed.

source("DSS.R")
lapply(c("matrixStats","reshape","reshape2", "scales", "drc", "caTools","data.table", "MESS", "BiocManager","svMisc", "egg", "pheatmap", "sva", "pcaMethods","tidyverse"), require, character.only = T)
...
CALC_METRICS <- function(dose_responses, dose_responses_grouped, graph = F, path = ""){
  start.time <- Sys.time()
  ...
  for(it in 1:iter){
    df.DSS.ctx <- CALC_IC50_EC50_DSS(it, dose_responses_grouped, dose_responses, DSS_typ = '2', graph = graph, path=path)
    
    if (length(df.DSS.ctx) < 9){
      list.believe[[it]] <- data.frame(believe_DSS = NA)
      list.AUC_DSS[[it]] <- data.frame(AUC = NA, DSS1 = NA,
                                     DSS2 = NA, DSS3 = NA)
      list.IC50[[it]] <- data.frame(IC50 = NA)
    } else {
...
## modified: returns a smaller dataframe and QC conditions
#' @param path logical T/F, directory to save graph outputs
#' @param failed logical T/F, to save curves that failed QC (background is colored red)
CALC_METRICS.2 <- function(dose_responses, dose_responses_grouped, graph = F, path = "", failed = F){
  start.time <- Sys.time()
  iter <- nrow(dose_responses_grouped)
  list.AUC_DSS <- list()
  list.believe <- list()
  ...
  for(it in 1:iter){
    df.DSS.ctx <- CALC_IC50_EC50_DSS(it, dose_responses_grouped, dose_responses, DSS_typ = '2', graph = graph, path=path, failed = failed)

    if (length(df.DSS.ctx)<10){
      list.believe[[it]] <- data.frame(believe_DSS = NA)
      list.AUC_DSS[[it]] <- data.frame(NA)
      list.cond[[it]] <- data.frame(cond = NA)
    } else{
      list.believe[[it]] <- data.frame(believe_DSS = df.DSS.ctx[[3]])
      list.AUC_DSS[[it]] <- df.DSS.ctx[[1]][,4:ncol(df.DSS.ctx[[1]])]
      list.cond[[it]] <- data.frame(cond = df.DSS.ctx[[10]])
    }
    rm( df.DSS.ctx)
    svMisc::progress(it/iter*100)
  }
  df.believe <- bind_rows(list.believe)
  df.AUC_DSS <- bind_rows(list.AUC_DSS)
  df.cond <- bind_rows(list.cond)

  df.metric <- cbind( dose_responses_grouped, df.AUC_DSS, df.believe, df.cond)
  message("Finished DSS computations in ", round(Sys.time() - start.time, 2), units(Sys.time() - start.time))
  return(df.metric)
  
}

## modified: calculation for 1 sample-drug testing
## modified: returns a smaller dataframe and QC conditions
CALC_METRICS.single <- function(dose_responses, dose_responses_grouped, graph = F, path = "", failed = F){
  unlink("./IC50", recursive = T) ;dir.create("./IC50")
  df.DSS.ctx <- CALC_IC50_EC50_DSS(1, dose_responses_grouped, dose_responses, DSS_typ = '2', graph = graph, path=path, failed = failed)
  if (length(df.DSS.ctx)<10){
    df.metric <- data.frame(believe_DSS = NA, DSS = NA, cond = NA)
  } else{
    df.metric <- data.frame(believe_DSS = df.DSS.ctx[[3]], DSS = df.DSS.ctx[[4]], cond = df.DSS.ctx[[10]])
  }
  return(df.metric)
}
...
#   colnames(inputdrug) <- inputdrug.map['drugname', !(is.na(inputdrug.map['drugname',]))]
#   message("Finished drug mapping in ", round(Sys.time() - start.time, 2), units(Sys.time() - start.time),  paste(', ' ,as.character(ncol(inputdrug)), 'drugs in our drug library') ) #prvsly is patient.dss / hardcoded
#   return(inputdrug)
# }
# 
# 
...
#     if (filename == "") {ggsave(paste0("./procedure1_step8_Breeze_DSS_", ceiling(ncol(df)*proportion), "drugs_heatmap.pdf"), p0, height = 10,width = 10)}else
...
#     if (filename == "") {ggsave(paste0("./procedure1_step8_Breeze_DSS_", ncol(df), "drugs_heatmap.pdf"), p0, height = 10,width = 10)}else
#     {ggsave(filename, p0, height = 10,width = 10)}
...
#     
#     if (filename == "") {ggsave("./procedure1_step8_Breeze_DSS_barplot.pdf", p0, height = 10, width = 10)}else
#     {ggsave(filename, p0, height = 10, width = 10)}
