...
# Libraries 
lapply(c(c("matrixStats","dplyr","reshape","reshape2", "scales", "drc", 
           "caTools", "ggplot2", "data.table", "stringr","MESS")), library, character.only = !0)
...
dss<-function(ic50,slope,max,min.conc.tested,max.conc.tested,y=10,DSS.type=2,concn_scale=1e-9){
  ...
  # y here is the min required response
  # DSS set 0 when didnt hit 50% inhibition;  when there is no slope/ constant; when max response < y
  cond_zero <- -1

  a=as.numeric(unname(max)) # upper asymptote, max response
  b=as.numeric(unname(slope)) # slope factor
  d=0 # min response, lower asymptote
  ic50 = as.numeric(unname(ic50)) # inflection point
  ...
  if(is.na(ic50)||is.na(b)||is.na(a)||is.na(Min.Conc)||is.na(Max.Conc)){
    ...
    dss<-0 # when didnt hit 50% inhibition
    cond_zero <- 12
  }
  else if(isTRUE(b==0)){
    dss<-0 # when there is no slope/ constant
    cond_zero <- 13
  }
  else{
    if(a>100){ a<-100  }
    ...
    if(a>y){ # if max resopnse if more than y-required
      if(y!=0){
        x1<-(c - ((log(a-y)-log(y-d))/(b*log(10)))) # x1 = predicted conc at y% response
  ...
  return (list(dss, cond_zero))
}

CALC_IC50_EC50_DSS <- compiler::cmpfun(function(i, drug_wells_, xpr_tbl, DSS_typ, readoutCTX = F, path = "", graph, failed=FALSE){
  ...
  tryCatch({
    ...
    viability = 100-inhibition; believe_ = TRUE;
    ...
    if(DSS_typ == "AUC"){
      ...
      dss_score1 <- round(as.numeric(dss(coef_ic50["IC50"],coef_ic50["SLOPE"],coef_ic50["MAX"],min_signal,max_signal, DSS.type=1)[[1]]),1);
      dss_score2 <- round(as.numeric(dss(coef_ic50["IC50"],coef_ic50["SLOPE"],coef_ic50["MAX"],min_signal,max_signal, DSS.type=2)[[1]]),1);
      dss_score3 <- round(as.numeric(dss(coef_ic50["IC50"],coef_ic50["SLOPE"],coef_ic50["MAX"],min_signal,max_signal, DSS.type=3)[[1]]),1);
      ...
      if (graph){
        ...
        icpl <- ggplot2::ggplot(mat_tbl, aes(logconc, inhibition2)) + scale_x_continuous(breaks=mat_tbl$logconc,labels=round(mat_tbl$dose,2)) +
          ...
          geom_vline(xintercept = log10(coef_ic50["IC50"]), colour="grey", size = 0.8) + ggtitle(paste0(strtrim(drug_name, 30)," (dss:",dss_score,")\n"))+
      ...
      IC50_dataframe <- data.frame(ID=product_id,DRUG_NAME=drug_name,ANALYSIS_NAME="IC50", t(as.matrix(coef_ic50)), perInh,
                                   GRAPH=NA, DSS = as.numeric(dss_score), curve_resid = paste(round(sumIC50$residuals,2), collapse = ",", sep = ","), SE_of_estimate = as.numeric(ic50std_resid),AUC=auc)
      #dataframe for EC50
      ...
      # if(sum(abs(resid_)>20)> outlier-1) {believe_ = !1; cond = 2};
      # # if(tail(resp_,2)[[1]]-tail(resp_,1) > 7 && tail(resp_,1)-tail(resp_,3)[[1]] > 7) {believe_ = !1; cond = 3}
      # if((sum(abs(resid_)>15)>= outlier-1) && (ic50std_resid>15)) {believe_ = !1; cond = 5};
      # if(ic50std_resid>100) {believe_ = !1; cond = 6}
      # if(sum(abs(resid_)>15)> outlier){believe_ = !1; cond = 8};
      # if(sum(abs(resid_)>15)> outlier-1 && sum(abs(resid_)>20)>= outlier-1) {believe_ = !1; cond = 9};
      ...
      if (failed & !believe_){
        ...
        icpl <- ggplot2::ggplot(mat_tbl, aes(logconc, inhibition2)) + scale_x_continuous(breaks=mat_tbl$logconc,labels=round(mat_tbl$dose,2)) +
          ...
          theme_bw() + labs(y = "% inhibition", x = "conc(nM)")  +  ylim(-25, 125) +
          geom_text(mapping=aes(x2,y2,label = text2), data=data.frame(x2=log10(coef_ic50["IC50"])*0.95, y2=115, text2="IC50"), color="grey", parse=T) +
          theme(plot.background = element_rect(fill = "#ffecf2",colour = NA),
                panel.background =element_rect(fill = "transparent",colour = NA), 
                plot.title = element_text(hjust = 0.5, size = 12.5),
                axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1)
                )
